...
Now when the user generates meeting notes, an additional JSON file is sent to the AWS S3 bucket containing info about [each chosen recipient](#choosing-recipients). Using this info, an AWS Lambda requests text generations "tailored" to each recipient from the OpenAI API.
...
In MeetingChauffeur, there are two kinds of recipients: **Individuals** and **Groups**. Making use of these two kinds of recipients, the user can make new individuals, organize them into groups, and easily edit information within both.
